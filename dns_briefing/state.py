...
class StateDB:
    ...
    def update_known_domains(
        self, domains: list[str], today: date, window_days: int
    ) -> list[str]:
    ...
    def get_volume_baseline(
        ...
        rows = self._con.execute("""
            ...
            WHERE date >= ? AND date < ?
            GROUP BY client_ip
        """, [window_start, reference_date]).fetchall()

        return {
            row[0]: {"mean": row[1], "stddev": row[2] or 0.0, "days": row[3]}
            for row in rows
        }
